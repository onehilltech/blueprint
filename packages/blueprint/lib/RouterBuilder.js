...
RouterBuilder.prototype.addSpecification = function (spec, currPath) {
  ...
  function processVerb (verb, currPath, opts) {
    ...
    function defineResource (path, opts) {
      ...
      var actions = ['create', 'delete', 'getAll', 'getOne', 'update', 'count', 'first', 'last'];
      ...
      if (!resourceId)
        throw new Error ('Resource controller must define resourceId property');
      ...
      if (actions.indexOf ('last') !== -1)
        spec['/last'] = {get: makeAction (controllerName, 'getLast', opts.options)};
