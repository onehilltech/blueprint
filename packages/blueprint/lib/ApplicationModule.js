...
function ApplicationModule (appPath) {
  ...
  this._messenger = this._messaging.getMessenger ();
...
ApplicationModule.prototype.__defineGetter__ ('models', function () {
  ...
  this._messenger.emit ('appmodule.models.loading', this);

  this._models = Loader.loadModels (path.join (this._appPath, 'models'));

  this._messenger.emit ('appmodule.models.loaded', this);
  return this._models;
});
...
ApplicationModule.prototype.__defineGetter__ ('listeners', function () {
  ...
  this._messenger.emit ('appmodule.listeners.loading', this);

  // Load all the controllers into memory.
  var listenersPath = path.join (this._appPath, 'listeners');
  this._listeners = Loader.loadListeners (listenersPath, this._messaging);

  this._messenger.emit ('appmodule.listeners.loaded', this);
  return this._listeners;
});
