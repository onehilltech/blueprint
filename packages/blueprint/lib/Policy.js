...
function PolicyRule () {
  ...
  function findPolicyByName (name) {
    // Get the policy manager. If there is a module in the name, then we must
    // look in the policy manager for that module.
    var parts = name.split ('::');
    var pm = parts.length === 1 ? app.policies : app.modules[parts[0]].policies;
    var policyName = parts.length === 1 ? parts[0] : parts[1];

    return pm.find (policyName)
  }

  if (typeof func === 'string') {
    func = findPolicyByName (func);
