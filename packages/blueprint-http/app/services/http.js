...
module.exports = Service.extend({
  init() {
    this._super(...arguments); // We are going to listen for the module loaded message. This will allow
  ...
  async configure(app) {
    ...
    app.import(resources); // Now, we are going to create a router from the routers defined in the application
  ...
  async _handleModuleLoaded(module) {
    ...
    const resources = await this._httpLoader.load(module.modulePath); // Import the resources into the application module, and let's import the
