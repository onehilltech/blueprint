...
const {
  spawn
} = require('child_process'); // set the globals
...
module.exports = class DfinityService extends Service {
  constructor(app) {
    super(...arguments); /// A collection of named agents for connecting to the Internet Computer.

    _defineProperty(this, "defaultIdentity", void 0);

    Object.defineProperty(this, 'canisters', {
      writable: false,
      value: {}
    });
  ...
  async configure() {
    ...
    await fs.ensureDir(`${this.app.tempPath}/dfinity`); // Load the default identity for the application. The private key takes precedence
    ...
    if (privateKey !== false) {
      ...
      if (isString(privateKey)) this.defaultIdentity = await this._loadIdentity(`key://${privateKey}`);else this.defaultIdentity = await this._loadDefaultIdentity();
    } else if (phrase) {
  ...
  async createHttpAgent(options, fetchRootKey = true) {
    const opts = Object.assign({}, options); // The source agent can be a named agent. If it is a named agent, replace the
    // name (or string) with an agent.

    if (opts.source && isString(opts.source)) opts.source = this._lookupAgent(opts.source); // Load the identity if there is one defined. The identity could be a private
    ...
    const agent = new HttpAgent(opts); // Needed for update calls on local dev env, shouldn't be used in production!

    if (env !== 'production' && fetchRootKey) await agent.fetchRootKey();
    return agent;
  }
  /**
  ...
  async _loadIdentity(descriptor) {
    ...
    if (parts.length !== 2) return descriptor;
    let [protocol, filename] = parts;

    if (protocol === IDENTITY_KEY_PROTOCOL) {
      // The identity is a private key.
      if (!path.isAbsolute(filename)) filename = path.resolve(this.app.appPath, filename);
  ...
  _lookupAgent(name, failIfNotFound = true) {
    ...
    if (!agent && failIfNotFound) throw new Error(`The agent ${name} does not exist.`);
    return agent;
  }
  /**
  ...
  async _loadDefaultIdentity() {
    ...
    if (exists) return this._loadIdentity(descriptor);
    const privateKeyBuffer = await generatePrivateKey();
    await fs.writeFile(privateKeyFile, privateKeyBuffer); // The private key file does not exist. We need to create a new one for this node
