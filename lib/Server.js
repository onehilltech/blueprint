...
function Server (appPath, config) {
  ...
  // Copy all the views in the view cache path.
  this.importViews (this._viewCachePath.path);
 ...
 * @param path
 */
Server.prototype.importViews = function (path) {
  winston.log ('debug', 'importing views from %s', path);
  fse.copySync (path, this._viewCachePath.path, {recursive : true});
};
