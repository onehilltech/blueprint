  ...
  function processVerb (verb, currPath, opts) {
    ...
    function defineResource (path, opts) {
      ...
      if (actions.indexOf ('getAll') != -1)
        spec['get'] = makeAction (controllerName, 'getAll');

      if (actions.indexOf ('create') != -1)
        spec['post'] = makeAction (controllerName, 'create');

      if (actions.indexOf ('getOne') != -1)
        spec[individualPath]['get'] = makeAction (controllerName, 'get');

      if (actions.indexOf ('update') != -1)
        spec[individualPath]['put'] = makeAction (controllerName, 'update');

      if (actions.indexOf ('delete') != -1)
        spec[individualPath]['delete'] = makeAction (controllerName, 'delete');
