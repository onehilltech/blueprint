...
Object.defineProperty (PolicyError.prototype, 'firstError', {
  get () {
    let firstError = this._errors[0];
    return new HttpError (403, firstError.reason, firstError.message)
  }
});

Object.defineProperty (PolicyError.prototype, 'lastError', {
  get () {
    let error = this._errors[this._errors.length - 1];
        ...
        try {
          policy (req, policyCallback (req, function (err, result) {
            ...
            if (req.hasPolicyErrors)
              return handleError (req.policyError.lastError, res);
