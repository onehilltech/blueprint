...
Application.prototype.init = function (callback) {
  ...
  async.waterfall ([
    ...
    function (app, callback) {
      var appModule = new ApplicationModule (app._appPath);
      var modules = appModule.modules;

      async.eachSeries (modules, function (name, callback) {
        app.addModule (name, callback);
      }, function (err) {
 ...
 * @param callback
 */
Application.prototype.addModule = function (name, location, callback) {
  if (!callback) {
    callback = location;
    location = undefined;
  }
  ...
  if (!location)
    location = path.resolve (this._appPath, '../node_modules', name, 'app');
  ...
  async.waterfall ([
    ...
    // Load the sub-modules for this module.
    function (app, callback) {
      var modules = appModule.modules;
