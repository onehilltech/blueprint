...
Application.prototype.start = function (done) {
   ...
   * @param dir
   */
  function seedDatabaseFromPath (db, path, done) {
    winston.log ('info', 'seeding database: ' + path);
    ...
    for (var collection in seeds) {
      var seed = seeds[collection];

      db.seed (collection, seed, function (err, seed) {
        if (err) throw err;
        return done (null);
      });
    }
  ...
  this._db.connect (function (err) {
    ...
    async.each (paths, function (path, callback) {
      seedDatabaseFromPath (self._db, path, callback);
    }, function (err) {
      if (err) return done (err);
