...
const RouterBuilder = CoreObject.extend ({
  basePath: '/',
  ...
  build () {
    return new Promise ((resolve, reject) => {
      ...
      forOwn (this.routers, (definition, name) => {
        ...
        this.addRouterSpecification (this.basePath, name, definition);
   ...
   * @param spec
   */
  addRouterSpecification (route, name, spec) {
    ...
    if (spec.use)
      this._processUse (route, spec.use);
    ...
    allowed.forEach (function (actionName) {
      ...
      function processAction (action) {
        ...
        if (!actionOption.policy) {
          ...
          actionOption.policy = `${prefix}${name}.${actionName}`;
        }
  ...
  _makePolicyMiddleware (opts) {
    ...
    if (isString (opts)) {
      ...
      if (policy)
        middleware.push (checkPolicy (opts, policy));
    }
    else if (isArray (opts)) {
      ...
      if (policy)
        middleware.push (checkPolicy (name, policy, params));
    }
    else {
