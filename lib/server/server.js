...
const {merge,forOwn,forEach} = require ('lodash');
...
module.exports = Object.extend ({
  /// The application hosting the server.
  app: null,
   ...
   */
  configure (config) {
    return this._configureMiddleware (config).then (() => {
      return this._configureProtocols (config);
    }).then (() => {
      return this._configureStaticPaths (config);
    }).then (() => {
      return this._configureViews (config);
    }).then (() => {
      return this._finishConfigure (config);
    });
  },
  ...
  _configureBodyParserMiddleware (config = {}) {
    ...
    forOwn (bodyParserConfig, (config, type) => {
      ...
      if (middleware) {
        ...
        console.error (`*** warning: ${type} is an unsupported bodyParser type`)
      }
    });
  },
  ...
  _configureOptionalMiddleware (config) {
    ...
    forOwn (optional, (configurator, kind) => {
      let optionalConfig = config[kind];
  ...
  _configureProtocols (config) {
    return new Promise ((resolve) => {
      forOwn (config.protocols, (value, key) => {
        ...
        this._protocols[key] = Protocol.createProtocol (this.app, value);
